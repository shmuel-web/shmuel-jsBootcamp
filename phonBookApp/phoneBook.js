...
var person = function (id, fName, lName, phoneNum, group){
    this.id = id;
    this.fName = fName;
    this.lName = lName;
...
function addNewPerson(fName, lName, phoneNum) {
    var contact = new person(++personId, fName, lName, phoneNum, currentGroup);
...
function addNewGroup (name){
    ...
    var nGroup = new group(++groupId, name, currentGroup);
    groups.push(nGroup);
}
    ...
    while ( options [index -1] !== 'Exit' ) {
        ...
        switch (options[index -1]) {
            ...
            case 'Change current group':
                var groupName = readlineSync.question('please type group name:');
